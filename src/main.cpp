...
const uint16_t smartEnterCount = 16;   // 16 * 0.5s = 8s effective stable window
const float smartExitErrC = thermocoupleAccuracyC * 4.0f;  // 1.00 C
const uint16_t smartExitCount = 8;     // 8 * 0.5s = 4s outside band before fallback
...
void printConfig() {
  ...
  Serial.printf("CFG KP: %.3f | KI: %.3f | KD: %.3f | BIAS: %.2f | SPBIAS: %.2f | SP: %.2f | ALPHA: %.3f | MAXSTEP: %d | FAN: %.1f | MODE: %s | MANPWM: %.2f\n",
                kp, ki, kd, pid.getBias(), setpointBias, setpoint, emaAlpha, maxPwmStep, fanSpeedPercent,
                modeToText(controlMode), manualPwmTarget);
}
...
void handleCommand(char *line) {
  ...
  if (!command || strcmp(command, "SET") != 0) {
    Serial.println("ERR Unknown command. Use SET <KP|KI|KD|BIAS|SPBIAS|SP|ALPHA|MAXSTEP|FAN|MODE|MANPWM> <value> or GET");
    return;
  }
  ...
  if (!key || !valueText) {
    Serial.println("ERR Usage: SET <KP|KI|KD|BIAS|SPBIAS|SP|ALPHA|MAXSTEP|FAN|MODE|MANPWM> <value>");
    return;
  }
  ...
  if (strcmp(key, "KP") == 0) {
    ...
    Serial.println("ERR Unknown key. Use KP, KI, KD, BIAS, SPBIAS, SP, ALPHA, MAXSTEP, FAN, MODE, MANPWM");
    return;
  }
...
void setup()
{
  ...
  Serial.println("Commands: GET, SET KP <v>, SET KI <v>, SET KD <v>, SET BIAS <v>, SET SPBIAS <v>, SET SP <v>, SET ALPHA <v>, SET MAXSTEP <v>, SET FAN <0..100>, SET MODE <AUTO|MANUAL|SMART>, SET MANPWM <0..255>");
  printConfig();
}
...
void loop() {
  ...
  Serial.printf("Rawtemp %.2f C | Temp: %.2f C | Smooth: %.2f C | PWM: %d | P: %.2f | I: %.2f | D: %.2f | OUT: %.2f | BIAS: %.2f | SPBIAS: %.2f | SP: %.2f | EFFSP: %.2f | FAN: %.1f | FANPWM: %d | MODE: %s | STATE: %s | MANPWM: %.2f | HOLDPWM: %.2f | ENTPROG: %.1f | EXTPROG: %.1f | EABS: %.2f\n",
                rawTemp, t, tSmooth, pwm, pTerm, iTerm, dTerm, pidOut, pidBias, setpointBias, setpoint, effectiveSetpoint, fanSpeedPercent, fanPwmRaw,
